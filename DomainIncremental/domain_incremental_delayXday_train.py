...
# --------------- #
'''
模拟数据收集时间非常晚的实验 --> 对旧数据的更新能力
train——dataset 晚一个version
'''
DELAY = 2
# -------------------------------------------- #
...
for TASK in range(len(all_data_list)-DELAY):
    ...
    train_data = all_data_list[TASK]
    train_label = all_label_list[TASK]
    ...
    # test
    # '''all time data test'''
    # Acc, predictions, label_list = NMEClassifier(classifie_data=test_data_per_TASK,classifie_data_label=test_label_per_TASK,exemplars_set=exemplars_set,net=net,n_classes=NUM_CLASSES,device=DEVICE)
    # ACC_list.append(Acc)
